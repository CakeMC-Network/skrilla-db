package net.cakemc.skrilla.natives.file.checksum

import com.sun.jna.Native

object CheckSumUtility {
    private val library: CheckSumLibrary = Native.loadLibrary(
        if (System.getProperty("os.name").contains("Windows", ignoreCase = true)) {
            "checksum"
        } else {
            "lib_checksum"
        },
        CheckSumLibrary::class.java
    )
    ...
    fun calculateFileSum(filename: String): String {
        ...
        val result = library.calculate_file_sum(filename, checksum)
        if (result != 0) {
    ...
    fun validateFile(filename: String, expectedChecksum: String): Boolean {
        val result = library.validate_file(filename, expectedChecksum)
        return result == 1
    }
}